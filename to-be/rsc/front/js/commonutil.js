...
//					result.name = "박구글";
	...
	//			return appAccessToken;
				tempsend("/api/temp/getAccessToken", "GET", null, null, false)
	...
	//			return appRefreshToken;
				tempsend("/api/temp/getRefreshToken", "GET", null, null, false)
				.done(function(result) {
	//				return result.data;
					dfd.resolve(result.data);
				});
	...
	//			appRefreshToken = paramObj.refreshToken;
			...
			case "ReadProfileOf" :
				var paramObj = JSON.parse(params);
				var url = "/user/profile?memberNo="+paramObj.memberNo;
				getPageAjax(url)
				.done(function(result) {
...
function fn_getHpVal(val) {
	try {
		return val.replace(/[^0-9]/g, "").replace(/(^01[0-9])([0-9]*)([0-9]{4})/,"$1-$2-$3").replace("--", "-");
	} catch(e) {
		return val;
	}
}
...
function openGlobalSearchPopup(){	
	getPageAjax("/search/search_layer", null)
	.done(function(result) {
		$("body").find(".wrap").hide();
		$("body").append(result);
	});
...
function openChallengeDetail(chlgNo){	
	getPageAjax("/rank/challenge_layer?chlgNo="+chlgNo, null)
	.done(function(result) {
		$("body").find(".wrap").hide();
		$("body").append(result);
	});
